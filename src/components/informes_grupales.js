...
async function generarInformePDF(data) {
  ...
  if (data.estado === 'completado') {
    ...
    txt2 = `Durante el dictado de las clases se utilizaron diferentes herramientas metodológicas para abordar la trayectoria formativa.\n\nFue posible abordar la totalidad de los contenidos previstos del módulo ${data.modulo ? sanitize(data.modulo.nombre) : ''}, logrando alcanzar los objetivos pedagógicos propuestos para este período.`;
    txt3 = 'La regularidad de las clases y la asistencia del grupo permitieron alcanzar las metas educativas previstas sin inconvenientes.';
  } else {
    ...
    txt2 = `Durante el desarrollo de las clases se utilizaron diferentes recursos metodológicos para fortalecer el aprendizaje.\n\nDebido a la extensión de los contenidos del módulo ${data.modulo ? sanitize(data.modulo.nombre) : ''} y a la organización institucional, se completará el programa durante el próximo período.`;
