...
async function generarPDF(data) {
  ...
  if (encabezado && encabezado.data) {
    ...
    const innerY = marginTop + (imgHeight * 0.055); // Bajamos un poco más para que la línea negra superior quede bien gruesa y visible.
    const innerH = imgHeight * 0.81; // Compensamos restando un poquito de altura.
    
    // Relleno blanco transparente para tapar el texto y el casco originales sin sobreescribir bordes externos
    doc.setFillColor(255, 255, 255);
    doc.rect(innerX, innerY, innerW, innerH, 'F'); // Solo Fill ('F'), nada de Draw
    ...
    const numLines = doc.splitTextToSize(modTitle.toUpperCase(), innerW - 4);
  ...
  const paginaHTML = `
    <div class="acta-pagina">
      <div style="position:relative; width:100%;">
        ...
        <div style="position:absolute; top:6%; left:69.8%; width:28.2%; height:80%; background:#fff; display:flex; flex-direction:column; justify-content:center; align-items:center; text-align:center; padding: 2px;">
